...
public class DocumentSearchClient {
    ...
    public SearchResult search(String query, String userEmail, Long preferredDocumentId) {
        ...
        try {
            ...
            if (parsed.ambiguous()) {
                log.info("[RAG] ambiguous={} docs for exerciseRef='{}'", parsed.ambiguousDocuments().size(), parsed.exerciseRef());
                return new SearchResult(Collections.emptyList(), true, parsed.ambiguousDocuments(), parsed.exerciseRef());
            }
            log.info("[RAG] chunks={} preferredDocId={} user={}", parsed.results().size(), preferredDocumentId, userEmail);
            for (var chunk : parsed.results()) {
                log.info("[RAG] chunk docId={} file={} score={:.3f} page={} text={}",
                        chunk.documentId(), chunk.filename(), chunk.similarity(),
                        chunk.pageNumber(), chunk.chunkText().substring(0, Math.min(80, chunk.chunkText().length())));
            }
